  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/30 p-4">
      ...
      <div className="mt-3">
        ...
        <textarea value={features} onChange={(e) => setFeatures(e.target.value)} rows={3} className="mt-1 w-full rounded-md border border-border bg-card p-2 text-sm" />
      </div>
...
function Orgs() {
  ...
  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-base">Organizations</CardTitle><CardDescription>Every customer workspace.</CardDescription></CardHeader>
          ...
          <tbody>
            {orgs.map((o) => (
              <tr key={o.id} className="border-t border-border">
                ...
                <td className="px-4 py-2">{o.plan ?? "—"}{o.status ? <span className="ml-1 text-xs text-muted-foreground">({o.status})</span> : ""}</td>
...
function Gateways() {
  const [cfg, setCfg] = useState<any>({ stripe: { publicKey: "", secretKey: "", webhookSecret: "" }, razorpay: { keyId: "", keySecret: "" }, active: "" });
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  useEffect(() => { api.get<any>("/admin/settings/gateways").then((v) => v && setCfg((c: any) => ({ ...c, ...v }))).catch(() => {}); }, []);
  ...
  const upd = (gw: string, k: string, v: any) => setCfg((c: any) => ({ ...c, [gw]: { ...c[gw], [k]: v } }));

  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-base">Payment gateways</CardTitle><CardDescription>Keys are stored securely and used for checkout &amp; webhooks.</CardDescription></CardHeader>
      <CardContent className="space-y-5">
        <div>
          <div className="mb-2 text-sm font-semibold">Stripe</div>
          <div className="grid gap-2.5 sm:grid-cols-3">
            <Input placeholder="Publishable key" value={cfg.stripe?.publicKey ?? ""} onChange={(e) => upd("stripe", "publicKey", e.target.value)} />
            <Input placeholder="Secret key" type="password" value={cfg.stripe?.secretKey ?? ""} onChange={(e) => upd("stripe", "secretKey", e.target.value)} />
            <Input placeholder="Webhook secret" type="password" value={cfg.stripe?.webhookSecret ?? ""} onChange={(e) => upd("stripe", "webhookSecret", e.target.value)} />
          </div>
        </div>
        <div>
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold">PayPal
            <label className="flex items-center gap-1 text-xs font-normal text-muted-foreground"><input type="checkbox" className="h-3.5 w-3.5 accent-primary" checked={!!cfg.paypal?.live} onChange={(e) => upd("paypal", "live", e.target.checked)} /> Live mode</label>
          </div>
          <div className="grid gap-2.5 sm:grid-cols-3">
            <Input placeholder="Client ID" value={cfg.paypal?.clientId ?? ""} onChange={(e) => upd("paypal", "clientId", e.target.value)} />
            <Input placeholder="Client secret" type="password" value={cfg.paypal?.clientSecret ?? ""} onChange={(e) => upd("paypal", "clientSecret", e.target.value)} />
            <Input placeholder="Webhook ID" value={cfg.paypal?.webhookId ?? ""} onChange={(e) => upd("paypal", "webhookId", e.target.value)} />
          </div>
        </div>
